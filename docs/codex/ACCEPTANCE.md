...
| **Durable Auth & Profile** | Server-created ID, profile claim, session persistence |  Journey 1 Verified | `npm run smoke:session` proves opaque cookie persistence and rejects spoofed `x-metaedge-session-id` / raw user-id cookies. Profile ownership remains the next journey. |
...
| **Smoke Tests** | Local browser tests verify core user paths |  Partial | `npm run lint`, `npm run build`, and `npm run smoke:session` pass. Browser journey tests continue one path at a time. |
