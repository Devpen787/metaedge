...
| **Smoke Tests** | Local browser tests verify core user paths |  Partial | `npm run lint`, `npm run build`, `npm run smoke:session`, `npm run smoke:profile`, `npm run smoke:paper`, `npm run smoke:rooms`, and `npm run smoke:agents` pass. Browser journey tests continue one path at a time. |
