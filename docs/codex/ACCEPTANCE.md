...
| **Smoke Tests** | Local browser tests verify core user paths |  Partial | `npm run lint`, `npm run build`, `npm run smoke:session`, `npm run smoke:profile`, and `npm run smoke:paper` pass. Browser journey tests continue one path at a time. |
