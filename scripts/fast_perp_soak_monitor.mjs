...
while (Date.now() - startedAt < durationMs) {
  const [session, v5, health, processState, storage] = await Promise.all([endpoint('/api/session'),
    endpoint('/api/opportunity-factory/v5'), endpoint('/api/opportunity-factory/health'), processMetrics(), storageMetrics()]);
  const tiers = new Map((v5.body.economics?.topCandidates || []).map((contract) => [contract.id, contract.speedTier]));
  for (const decision of v5.body.economics?.recentShadowDecisions || []) {
  ...
  const sample = { at: Date.now(), session: { status: session.status, latencyMs: session.latencyMs },
    v5: { status: v5.status, latencyMs: v5.latencyMs, stale: v5.body.stale, liveExecution: v5.body.liveExecution,
      criticalFailures: v5.body.integrity?.criticalFailures?.length ?? null,
      contracts: Number(v5.body.economics?.counts?.contracts ?? 0), researchRuns: Number(v5.body.fastPerps?.counts?.researchRuns ?? 0),
      decisions: Number(v5.body.economics?.counts?.shadowDecisions ?? 0), outcomes: Number(v5.body.economics?.counts?.shadowOutcomes ?? 0) },
  ...
  if (prior && sample.v5.contracts > prior.v5.contracts && sample.v5.researchRuns <= prior.v5.researchRuns) {
    recordViolation('CONTRACT_CREATED_WITHOUT_RESEARCH_RUN', sample);
  }
  const recent = samples.filter((row) => row.at >= Date.now() - 5 * 60_000);
  const v5p95 = percentile(recent.map((row) => row.v5.latencyMs), 0.95);
  ...
  const automaticPause = automaticPauseReason({ stage, sample, recent, intervalMs,
    projectedTotalKbPerDay, v5p95Ms: v5p95 });
...
const metrics = { endpointFailures: samples.filter((row) => row.session.status !== 200 || row.v5.status !== 200
  ...
  v5P95Ms: percentile(samples.map((row) => row.v5.latencyMs), 0.95),
  v5P99Ms: percentile(samples.map((row) => row.v5.latencyMs), 0.99),
  ...
  liveExecutionLockedThroughout: samples.every((row) => row.health.currentLiveLock === true && row.v5.liveExecution === 'locked'),
...
const gates = {
  ...
  v5Latency: metrics.v5P95Ms != null && metrics.v5P95Ms < 250 && metrics.v5P99Ms != null && metrics.v5P99Ms < 500,
