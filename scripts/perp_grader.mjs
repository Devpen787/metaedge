...
export const LEGIBILITY = {
  ...
  notYet: [
    ...
    'No verdict below n=30 in the 40+ (extreme) band AND t>=2 — small samples are shown but never set FLAGS. Raised from n=20 (a real, unjustified inconsistency vs momentum/memecoin/stock\'s n>=30) during a cross-lane review: extreme-funding events are rarer, and a rarer signal deserves at least as high a bar as a common one, not a lower one.',
  ],
  why: [
...
async function main() {
  ...
  for (const [lo, hi, label] of bands) {
    ...
    const isFlag = lo >= 40 && g.length >= 30 && exp > 0 && t >= T_BAR;
    if (isFlag) flags++;
  ...
  console.log(`  Edge = a 40+ (extreme) band with n>=30, positive net, AND t>=2. Prior guarded: extreme funding may reflect real info, not just crowding.`);
