...
test('a validated queued decision routes exactly once through the canonical paper ledger', async () => {
  ...
  db.agents.agent_test = {
    ...
    assetSymbol: 'TEST', tradeType: 'token', strategyType: 'momentum', leverage: 1,
    status: 'active', createdAt: now, autopilot: true,
  };
  writeDatabase(db);
  const decision = persistDecision({
    id: 'decision_router_fixture', cycleId: 'cycle_fixture', evaluatedAt: now, symbol: 'TEST', instrument: 'spot',
    ...
    featureEvidence: [], validationStatus: 'forward_paper_candidate', queueStatus: 'queued', ownerId: 'user_test', agentId: 'agent_test',
  });
  const context: any = {
    routing: { ownerId: 'user_test', agentId: 'agent_test' },
    features: { 'price.v1': { value: 100 } },
  ...
  assert.equal(after.auditEvents[0].action, 'LAYERED_PAPER_TRADE');
});
