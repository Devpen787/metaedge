...
test('strategy, validation, decision, and execution state survive a database reread', async () => {
  ...
  const { persistStrategySpec, persistValidation, persistDecision, markDecisionRouted, decisionRuntimeSnapshot } = await import('../../server/decision/store.js');
  ...
  assert.equal(decisionRuntimeSnapshot().recentDecisions[0].queueStatus, 'routed');
